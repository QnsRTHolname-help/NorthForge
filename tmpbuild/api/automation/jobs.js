...
export default async function handler(req, res) {
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (error)
                throw new ApiError(500, 'DB_ERROR', 'Could not load jobs.');
            return ok(res, { jobs: data });
        }
        if (req.method === 'POST') {
            const body = req.body;
            if (!body.automationId)
                throw new ApiError(400, 'VALIDATION', 'automationId is required.');
            ...
            const tenantId = principal.role === 'client' ? principal.clientId : (body.clientId ?? '');
            if (!tenantId)
                throw new ApiError(400, 'VALIDATION', 'clientId is required.');
            ...
            const { data: def, error: defErr } = await db
                ...
                .single();
            if (defErr || !def)
                throw new ApiError(404, 'AUTOMATION_NOT_FOUND', 'Automation could not be found.');
            if (def.archived || !def.enabled) {
                throw new ApiError(409, 'AUTOMATION_UNAVAILABLE', 'This automation is currently unavailable.');
            }
            const version = def.automation_versions?.[0];
            if (!version)
                throw new ApiError(409, 'NO_ACTIVE_VERSION', 'This automation has no published version.');
...
export async function dispatchQueuedJobs() {
    const { hermes } = await import('../../server/lib/hermes.js');
    ...
    for (const job of queued) {
        ...
        try {
            ...
            await db.from('automation_logs').insert({
                ...
                message: err?.code === 'HERMES_NOT_CONFIGURED'
