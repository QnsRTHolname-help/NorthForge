...
import { ai, buildSummaryMessages, AiUnavailableError } from '../../../../server/lib/ai.js';
...
export default async function handler(req, res) {
    try {
        ...
        if (!id)
            throw new ApiError(400, 'VALIDATION', 'Job id is required.');
        ...
        assertTenant(principal, job.client_id);
        ...
        const cached = result.ai_summary;
        const summaryAt = result.ai_summary_at ? new Date(result.ai_summary_at).getTime() : 0;
        ...
        try {
            const { text, model } = await ai.complete(buildSummaryMessages(context, principal.role === 'admin' ? 'admin' : 'client'));
            await db
                .from('automation_results')
                .update({
                ai_summary: text,
