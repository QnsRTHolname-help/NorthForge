...
function rateLimited(ip) {
    ...
    hits.set(ip, list);
    if (hits.size > 5_000)
        hits.clear(); // crude memory guard
    return list.length > RATE_MAX;
}
const sanitize = (v, max) => typeof v === 'string' ? v.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, '').trim().slice(0, max) : '';
function buildMessages(p) {
    ...
    return [
        {
            role: 'system',
            content: `You are the customer-support AI assistant for the business described below. ` +
        ...
        { role: 'user', content: sanitize(p.message, MAX_MESSAGE) },
    ];
}
export default async function handler(req, res) {
    try {
        ...
        if (rateLimited(ip))
            throw new ApiError(429, 'RATE_LIMITED', 'Too many messages — please slow down.');
        const body = (req.body || {});
        ...
        if (!business)
            throw new ApiError(400, 'VALIDATION', 'Business context is required.');
        try {
