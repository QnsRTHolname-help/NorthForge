...
export default async function handler(req, res) {
    try {
        ...
        const check = verifyCallbackSignature(rawBody, req.headers['x-hermes-signature'], env.hermesCallbackSecret(), req.headers['x-hermes-timestamp']);
        if (!check.valid) {
        ...
        if (!event?.type)
            throw new ApiError(400, 'VALIDATION', 'Event type is required.');
        ...
        if (!jobId)
            throw new ApiError(404, 'JOB_NOT_FOUND', 'No job matches this event.');
        ...
        if (resultRow)
            resultRow.job_id = jobId;
        for (const f of fileRows)
            f.job_id = jobId;
        if (resultRow)
            await db.from('automation_results').upsert(resultRow, { onConflict: 'job_id' });
        if (fileRows.length) {
