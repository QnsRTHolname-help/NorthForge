...
export function requireEnv(name: string): string {
  ...
  if (!v) throw Object.assign(new Error(`Missing env ${name}`), { code: 'SERVER_MISCONFIGURED' });
  return v;
}
...
export const hermesConfigured = (): boolean =>
  Boolean(env.hermesApiUrl() && env.hermesApiKey());
