...
export interface DispatchInput {
  websiteJobId: string;
  hermesAutomationKey: string;
  input: unknown;
  config: unknown;
  callbackUrl: string; // absolute URL of /api/hermes/events
...
export interface HermesAdapter {
  ...
  /** Submit a job to Hermes. MUST return the real Hermes job id. */
  dispatch(job: DispatchInput): Promise<{ hermesJobId: string }>;
  /** Request cancellation. Only callable when Hermes actually supports it. */
  cancel(hermesJobId: string): Promise<void>;
  /** Real health probe (heartbeat endpoint) — never guessed. */
  health(): Promise<HermesHealth>;
}

class UnconfiguredHermesAdapter implements HermesAdapter {
  readonly name = 'unconfigured';
  async dispatch(): Promise<{ hermesJobId: string }> {
    throw new ApiError(
      503,
      'HERMES_NOT_CONFIGURED',
      'Hermes integration is not yet configured. Your job is safely queued and will be dispatched once the automation engine is connected.',
    );
  }
...
export function verifyCallbackSignature(
  ...
  if (!signature || !timestampHeader) return { valid: false, reason: 'missing signature headers' };
  const ts = Number(timestampHeader);
  if (!Number.isFinite(ts)) return { valid: false, reason: 'bad timestamp' };
  if (Math.abs(Date.now() - ts) > maxSkewMs) return { valid: false, reason: 'timestamp outside window (replay?)' };
  const expected = createHmac('sha256', secret).update(`${ts}.${rawBody}`).digest();
  const given = Buffer.from(signature, 'hex');
  if (given.length !== expected.length) return { valid: false, reason: 'signature length mismatch' };
  return { valid: timingSafeEqual(given, expected) };
}

export const hermes: HermesAdapter = hermesConfigured()
